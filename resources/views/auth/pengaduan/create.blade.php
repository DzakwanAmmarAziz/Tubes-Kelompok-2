              ...
              <form action="/pengaduan" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-6 col-md-4 mb-3">
                    <label for="gedung_id">Gedung ID</label>
                    <select class="form-control" id="gedung_id" name="gedung_id" style="cursor: pointer;">
                      @if ($gedung->count() > 0)
                        <option value="" selected disabled>Pilih opsi</option>
                        @foreach ($gedung as $g)
                          <option value="{{ $g->id }}">{{ $g->id }} - {{ $g->nama_gedung }}</option>
                        @endforeach
                      @else
